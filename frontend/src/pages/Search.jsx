            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px,1fr))", gap: 14 }}>
              {results.map(r => {
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4, position: "relative", zIndex: 1, gap: 10 }}>
                      <div style={{ fontWeight: 800, fontSize: 16, color: "var(--heading-color)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", display: "flex", alignItems: "center", gap: 6, flex: 1 }}>
                        {name}
                    ...
                    <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        ...
                        {!isDoctor && +r.emergency === 1 && (
                          <div style={{ position: "absolute", bottom: -4, right: -4, background: "var(--danger-bg, #ef4444)", color: "var(--danger-text, #fff)", width: 18, height: 18, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", border: "1.5px solid var(--card-bg)", fontSize: 10 }}>
                            🚨
                          </div>
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          <Stars rating={Math.round(avgRating)} color="#0891b2" />
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, flex: 1 }}>
                      ...
                      <div style={{ fontSize: 13, color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: 6 }}>
                        {isDoctor ? <Building size={14} color="var(--text-muted)" /> : <Phone size={14} color="#0891b2" />}
                      ...
                      {!isDoctor ? (
                        <div style={{ display: "flex", flexDirection: "column", gap: 6, background: "var(--bg)", padding: "10px 12px", borderRadius: 14, border: "1px solid var(--border)" }}>
                          {email && (
                          ...
                          {website && (
                            <div style={{ fontSize: 11, color: "#0891b2", display: "flex", alignItems: "center", gap: 6, fontWeight: 600 }}>
                              <Globe size={12} /> {website.replace(/^https?:\/\//, '')}
                            </div>
                          )}
                          {fax && (
                            <div style={{ fontSize: 11, color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 6 }}>
                              <Phone size={12} color="var(--text-muted)" style={{ transform: "rotate(270deg)" }} /> <span style={{ opacity: 0.7 }}>Fax:</span> {fax}
                            </div>
                          )}
                      ...
                      {!isDoctor && (
                        <div style={{ display: "flex", gap: 12, marginTop: 4, alignItems: "center" }}>
                          <div style={{ fontSize: 11, color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 4 }}>
                            <Clock size={12} color="#0891b2" /> {r.experience || 7} {t("years")}
                          </div>
                    ...
                    {/* Fixed Bottom Bar */}
                    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--border)", background: "var(--card-bg)" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                        <Stars rating={Math.round(+avgRating)} size={13} color="#0891b2" />
                        <span style={{ fontSize: 11, color: "#0891b2", fontWeight: 700 }}>({ratingCount}) {t("reviews", "avis")}</span>
                        {!isDoctor && (
