...
- **التصحيح الشامل والكامل لبيانات الولايات والبلديات في الجزائر (Wilayas & Baladiyas Complete Remediation)**:
  - **المشكلة**: كانت العديد من الولايات تفتقر إلى جزء كبير من بلدياتها الرسمية (مثل ولاية المسيلة التي كانت تعرض 18 بلدية فقط بدلاً من 47، والجلفة 19 بدلاً من 36، والجزائر العاصمة 50 بدلاً من 57 مع غياب بلديات رئيسية مثل محمد بلوزداد، سيدي امحمد، عين بنيان، المحمدية، المرسى، معالمة، الحمامات)؛ والسبب يعود إلى فصل دوائر كبرى في ولايات وهمية (59 إلى 69) وحذف البلديات المتشابهة في الأسماء بين الولايات.
  - **الحل الجذري المطبق**:
    - إعادة ربط بلديات الدوائر والولايات المنتدبة (59 إلى 69) بولاياتها الأم الرسمية (مثل بوسعادة إلى المسيلة، وعين وسارة ومسعد إلى الجلفة، وأفلو إلى الأغواط، وبريكة إلى باتنة، وقصر البخاري إلى المدية، وبئر العاتر إلى تبسة، وقصر الشلالة إلى تيارت، والبيض، وتلمسان، وبسكرة).
    - دمج البلديات المكررة داخل نفس الولاية وتحديث كافة ارتباطات الأطباء (13,908 طبيب) والمرضى لضمان عدم فقدان أو كسر أي ارتباط جغرافي.
    - إدراج كافة البلديات الـ 64 الناقصة مع معرفات فريدة UUIDv4 ورموز بريدية رسمية صحيحة تابعة لبريد الجزائر.
    - إزالة السجلات الزائدة وحصر قائمة الولايات في الـ 58 ولاية الرسمية للجمهورية الجزائرية.
    - **النتيجة**: مطابقة 100% لكافة ولايات الجزائر الـ 58، مع إتاحة الـ 1,541 بلدية كاملة دون أي نقص أو تكرار عبر الـ API وقوائم الاختيار والبحث.
...
### Fixed
- **إصلاح محاذاة وسام التميز (Award Icon) واتجاه البطاقات عند التبديل بين اللغات (RTL / LTR)**:
  - **المشكلة**: عند التبديل من العربية (RTL) إلى الفرنسية أو الإنجليزية (LTR)، كانت شارة التميز (`<Award />`) تظل مثبتة بموضع مطلق على اليسار (`left: 0`)، وكان اسم الطبيب يظل مجبراً على اليمين بواسطة `justifyContent: "flex-end"`.
  - **الحل في `frontend/src/App.jsx` (`SearchPage`)**:
    - اعتماد تخطيط Flexbox ديناميكي ثنائي الاتجاه (`justifyContent: "space-between"`) ينعكس تلقائياً حسب اتجاه الصفحة.
    - في وضع LTR (الفرنسية / الإنجليزية): يظهر اسم الطبيب في جهة اليسار والشارة في جهة اليمين.
    - في وضع RTL (العربية): يظهر اسم الطبيب في جهة اليمين والشارة في جهة اليسار بدون أي تداخل أو إزاحة شاذة.
    - جعل الشريط الجانبي اللوني للبطاقة ديناميكياً (`borderLeft` في LTR و `borderRight` في RTL).
...
- **إنشاء وتفعيل حساب فريق الدعم للتحكم في قبول طلبات الأطباء والعيادات (`Support Role - usertype = 4`)**:
  - **إنشاء مستخدم الدعم في قاعدة البيانات (`users` table)**:
    - إنشاء الحساب بالبيانات المحددة: اسم المستخدم `support`، كلمة المرور `amar1990` المشفرة عبر خوارزمية `Bcrypt`، ونوع الحساب `usertype = 4`.
  - **تحديث وضبط الصلاحيات البرمجية في الخادم الخلفي (`Backend RBAC & API`)**:
    - **`backend/middleware/AuthMiddleware.php`**: توسيع دالة `adminOnly()` للتحقق من `in_array($usertype, [3, 4], true)` للسماح لفريق الدعم (`usertype = 4`) بالوصول الكامل لخدمات إدارة طلبات العيادات والأطباء والإحصائيات وتذاكر الدعم.
    - **`backend/controllers/AuthController.php`**: دعم `usertype = 4` في دالتي تسجيل الدخول `login()` واسترجاع الملف الشخصي `me()`، وتعيين وسام `support` واسم المستخدم.
    - **`backend/controllers/TicketController.php`**: شمول مستخدم الدعم (`usertype = 4`) بصلاحيات متابعة التذاكر والرد عليها وإغلاقها.
  - **تحديث واجهات المستخدم الأمامية والتوجيه (`Frontend Navigation, Routes & Locales`)**:
    - **`frontend/src/App.jsx`**:
      - تضمين خيار "لوحة الإدارة" (`/admin`) في القائمة المنسدلة للترويسة للمستخدمين من نوع `4` بالإضافة إلى `3`.
      - تحديث مسار التوجيه المحمي لصفحة الإدارة `/admin` للسماح بـ `user.user_type === 4`.
      - تهيئة واجهة الملف الشخصي `ProfilePage` لعرض وسام "فريق الدعم" وزر الوصول المباشر للوحة الإدارة ومراجعة طلبات الأطباء والعيادات.
    - **ملفات الترجمة (`ar.json`, `fr.json`, `en.json`)**: إضافة مفتاح `support_role` ("فريق الدعم" / "Équipe Support" / "Support Team").


- **الدعم الكامل للغات والترجمة الشاملة لجانب الإدارة (`Admin Multi-Language Internationalization - AR / FR / EN`)**:
  - **تعريب وترجمة لوحة تحكم الإدارة بالكامل (`AdminDashboardPage` في `frontend/src/App.jsx`)**:
    - استبدال كافة النصوص الثابتة والمشفرة باستدعاءات مكتبة `react-i18next` عبر `t(...)` مع قيم افتراضية متوافقة.
    - ترجمة الترويسة، التبويبات الرئيسية، والتبويبات الفرعية (قيد المراجعة، الأرشيف المعتمد، سجل المرفوضات، كافة السجلات).
    - ترجمة بطاقات الإحصائيات وزيارات الموقع (اليوم، الأسبوع، الشهر، الإجمالي، حسب الدولة والولاية).
    - تعريب وترجمة شريط الأدوات وعناصر التحكم (حقول البحث المخصصة للأطباء والعيادات، فلاتر الفترات الزمنية، عدد السجلات، التبديل بين الجدول والبطاقات، وتصدير ملفات CSV).
    - ترجمة ترويسات جدول البيانات، شارات الحالات، أزرار الإجراءات الفردية، وشريط المعالجة الجماعية (`Bulk Actions`).
    - ترجمة نافذة تفاصيل الطلب المنبثقة (`Inspection Modal`) ونافذة توضيح أسباب الرفض (`Reject Modal`).
    - تعريب شريط ترقيم الصفحات (`Pagination`) ودعم التبديل التلقائي لاتجاه الأسهم والأيقونات (Chevron Left/Right) وتنسيق التواريخ وفق لغة المستخدم (`ar-DZ`, `fr-FR`, `en-US`).
  - **ترجمة واجهة الملف الشخصي للمشرف العام (`Admin Profile` في `ProfilePage`)**:
    - ترجمة شارات الرتبة والصلاحيات ("مدير النظام" / "صلاحيات كاملة").
    - ترجمة بطاقات الإحصاءات الخمس (الأطباء، العيادات، المرضى، المواعيد، التذاكر) وشارات التنبيه الحية (+معلق، اليوم، مفتوحة).
    - ترجمة تنبيه طلبات الانضمام الجديدة وزر المراجعة الفورية.
    - ترجمة بطاقات الوصول السريع الثلاث (لوحة الإدارة، مركز الدعم، إعدادات الأمان).
    - ترجمة ويدجت أحدث تذاكر الدعم وحالاتها والرسائل وأطراف المحادثة، وإرشادات الأمان.
  - **تحديث قواميس اللغات الثلاث (`frontend/src/locales/ar.json`, `fr.json`, `en.json`)**:
    - إضافة أكثر من 60 مفتاح ترجمة متطابق بدقة في اللغات العربية والفرنسية والإنجليزية لضمان تناغم تام وتجربة مستخدم عالمية عالية الجودة.

- **نظام إدارة وأرشفة طلبات الأطباء والعيادات واسع النطاق وإعادة التنظيم الذكي (`Admin Requests & Smart Archive`)**:
  - **التوسع لمعالجة آلاف الطلبات البرمجية في الخلفية (`backend/controllers/AdminController.php`)**:
    - تطوير دالتي `listClinics()` و `listDoctors()` لتوفير:
      - تصفح مرن مع تقسيم الصفحات من جانب الخادم (`limit` قابل للتخصيص من 5 إلى 100، والافتراضي 15/20) وإزاحة `offset`.
      - محرك بحث نصي مباشر (`q` / `search`) يفحص في آن واحد الاسم، التخصص أو العنوان، البريد الإلكتروني، الهاتف، والعيادة المرتبطة.
      - فلترة زمنية دقيقة (`period`: اليوم، آخر 7 أيام، آخر 30 يوماً، هذا العام، كافة الفترات).
      - حساب إحصاءات الحالات الكلية في استعلام واحد سريع (`counts: { PENDING, APPROVED, REJECTED, TOTAL }`) لعرض أرقام دقيقة وحية على شارات التبويبات دون إبطاء الخادم.
      - دعم الفرز (`order_by`, `order_dir`) وفق تاريخ الإنشاء أو تاريخ الاعتماد.
  - **الأرشفة الذكية للطلبات المعتمدة والمرفوضة (`Smart Archive`)**:
    - فصل الطلبات المعتمدة تاريخياً في تبويب مخصص ("الأرشيف المعتمد" / `Approved Archive`) مع توثيق تاريخ وساعة الاعتماد الدقيقة لكل عيادة وطبيب.
    - الاحتفاظ بسجل المرفوضات مع سبب الرفض الموثق.
    - إبقاء طابور المراجعة ("قيد المراجعة" / `Pending Review`) مركزاً حصرياً على الطلبات التي تتطلب اتخاذ إجراء فوري من الإدارة.
  - **إعادة تصميم الواجهة الأمامية لنظام الإدارة (`AdminDashboardPage` في `frontend/src/App.jsx`)**:
    - **شريط تحكم متكامل وعالي الكثافة (Scalable Controls Toolbar)**:
      - حقل بحث سريع مع Debounce تلقائي وزر إفراغ فوري.
      - قائمة منسدلة لتحديد الفترة الزمنية.
      - قائمة منسدلة لاختيار عدد السجلات المعروضة في الصفحة (15، 25، 50، 100).
      - زر للتبديل بين **نمط جدول البيانات عالي الكثافة (Data Table)** و**نمط البطاقات الشبكية (Cards Grid)**.
      - زر تصدير النتائج الفورية إلى ملف CSV متوافق مع Excel واللغة العربية بفضل ترويسة UTF-8 BOM (`\uFEFF`).
      - زر تحديث البيانات اليدوي مع مؤشر دوران.
    - **إجراءات المعالجة الجماعية (Bulk Actions Bar)**:
      - إمكانية تحديد عدة طلبات قيد المراجعة دفعة واحدة مع خيار "تحديد الكل" وقبولها وتفعيل حساباتها جميعاً بنقرة واحدة.
    - **عرض جدول البيانات (High-Density Data Table)**:
      - أعمدة واضحة: معرف الطلب، الاسم مع شارة NIN، التخصص أو العنوان، معلومات الاتصال (مع أزرار نسخ البريد والهاتف بنقرة واحدة)، تاريخ التسجيل، تاريخ الاعتماد، وشارة الحالة.
      - زر استعراض كامل التفاصيل في نافذة منبثقة تفاعلية (`Inspection Modal`) تتيح نسخ البيانات ومعاينة الملاحظات وقبول/رفض الطلب.
    - **شريط التنقل بين الصفحات (Smart Pagination Bar)**:
      - نطاق عرض السجلات من إجمالي النتائج.
      - أزرار الانتقال (الأولى، السابق، ترقيم الصفحات بنطاق ذكي `Windowed Page Numbers`، التالي، الأخيرة).

- **إصلاح خطأ تذاكر الدعم (500 Internal Server Error) وتطوير واجهة الملف الشخصي للمشرف العام (`Admin Profile` & `TicketController.php`)**:
  - **إصلاح خطأ `GET /api/tickets` 500 في الخلفية البرمجية (`TicketController.php`)**:
    - كان مسار جلب التذاكر لا يعالج نوع المستخدم `usertype = 3` (المدير العام)، مما أدى لبقاء استعلام الـ SQL فارغاً وحدوث خطأ خادم داخلي 500 متكرر في الـ Console أثناء فحص الرسائل والتنبيهات.
    - تمكين المدير العام من استعراض كافة التذاكر عبر المنصة مع بيانات المريض والطبيب والعيادة والرسائل غير المقروءة.
    - تمكين المدير العام من قراءة والرد وإغلاق أي تذكرة مع تمييز صفته كـ `إدارة المنصة`.
  - **تطوير واجهة متكاملة للملف الشخصي لمدير النظام (`ProfilePage` في `App.jsx`)**:
    - إصلاح دالة `load()` لتفادي استدعاء ملف المريض الشخصي عند تسجيل دخول المشرف (الذي كان يرجع 404 خطأ).
    - إضافة بطاقة تعريفية مخصصة في الترويسة تتضمن أيقونة الشارع وحزام الألوان الملكي، شارات "مدير النظام" و"صلاحيات كاملة"، وأزرار وصول سريع إلى لوحة تحكم الإدارة ومركز التذاكر.
    - إضافة تبويبات مخصصة للأدمن:
      1. **تبويب نظرة عامة والمنصة**:
         - 5 بطاقات إحصائية حية: الأطباء المعتمدون، العيادات الشريكة، المرضى المسجلون، إجمالي المواعيد، وتذاكر الدعم.
         - شريط تنبيهي ذكي يوضح عدد طلبات الانضمام الجديدة المعلقة للأطباء والعيادات مع زر للمراجعة الفورية.
         - بطاقات وصول سريع لإدارة المنصة، مركز الدعم، وإعدادات الأمان.
         - ويدجت يعرض أحدث تذاكر الدعم مع حالة التذكرة والمرسل وآخر رسالة.
      2. **تبويب الأمان وبيانات الدخول**:
         - نموذج لتعديل اسم المستخدم وكلمة المرور الخاصة بحساب المدير العام مع تنبيه وإرشادات الأمان.
  - **تحديث نقطة نهاية تعديل بيانات الدخول (`PatientController::updateCredentials`)**:
    - تحويل فحص الصلاحيات من `patientOnly` إلى `authenticate` ليتمكن أي مستخدم مصادق عليه (بما فيهم مدير النظام) من تحديث بيانات دخوله بأمان في جدول `users`.
